...
export function PrimaryButton({
  ...
  onClick?: () => void;
  disabled?: boolean;
  /** W-35D mutes the in-flight "Saving…" button on mobile; the WD-35D
   *  desktop frame keeps it at full #08664d. */
  busy?: boolean;
}) {
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className={`flex h-[48px] w-full items-center justify-center rounded-[13px] bg-[#08664d] lg:h-[52px] lg:rounded-[12px] text-[13px] font-semibold text-white hover:bg-lp-button disabled:cursor-not-allowed ${
        busy
          ? "disabled:bg-[#8fbcab] lg:disabled:bg-[#08664d]"
          : "disabled:bg-[#8fbcab]"
      }`}
    >
      {label}
...
/** White pill with green bold label: 104×34 on the W-xx mobile frames
 *  (Figma 628:610), 180×36 on the desktop WD- frames. */
export function PillButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex h-[34px] w-[104px] items-center justify-center rounded-[13px] border border-[#d6e0da] bg-white text-[13px] font-semibold text-[#08664d] hover:bg-lp-tint lg:h-[36px] lg:w-[180px] lg:rounded-[12px]"
    >
      {label}
    </button>
  );
...
export function SheetShell({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center lg:items-center">
      ...
      />
      <div className="relative w-full rounded-t-[18px] border border-[#d6e0da] bg-white p-[20px] pb-[30px] lg:w-[520px] lg:p-[24px] lg:rounded-[18px] lg:pt-[21px] lg:pb-[69px]">
        {children}
      </div>
    </div>
  );
